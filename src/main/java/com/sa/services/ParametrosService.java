...
public class ParametrosService {
	...
	public String abmDelegaciones(RelacionUsuarioDelegadoForm formulario, Usuario user) throws TransactionException {
		...
		if (formulario.getOpcion().trim().equals("BAJA")) {
			...
			parametersExecute.put("fDesde_old", this.getFechaYYYY_MM_DD(formulario.getFeDesde()));
			parametersExecute.put("fHasta_old", this.getFechaYYYY_MM_DD(formulario.getFeHasta()));
		} else {
			parametersExecute.put("id_reemplazo", formulario.getDelegadoUser().trim());
			...
			if (formulario.getOpcion().trim().equalsIgnoreCase(ParamsConstants.SU81_MODIFICACION)) {
				...
				parametersExecute.put("fDesde", this.getFechaYYYY_MM_DD(formulario.getFeDesde()));
				parametersExecute.put("fHasta", this.getFechaYYYY_MM_DD(formulario.getFeHasta()));

				parametersExecute.put("fDesde_old", this.getFechaYYYY_MM_DD(formulario.getFeDesdeOld()));
				parametersExecute.put("fHasta_old", this.getFechaYYYY_MM_DD(formulario.getFeHastaOld()));
			}
		}
		manager.executeTrx(this.client, parametersExecute);

		if (manager.getMensajeAviso() != null && !((String) manager.getMensajeAviso()).equalsIgnoreCase("")) {
...
//		parametersExecute.put("cod_gasto", "0200");
	...
	public String modifRelacionUsuarioDelegado(String idusr, String delegado, String feDesde, String feHasta, String estInf, String estCarg)
		...
		Map<String, Object> parametersExecute = new HashMap<String, Object>();
		parametersExecute.put("opcion", "MODI");
		parametersExecute.put("", idusr);
		parametersExecute.put("", delegado);
		parametersExecute.put("", feDesde);
		parametersExecute.put("", feHasta);
		parametersExecute.put("", estInf);
		parametersExecute.put("", estCarg);
		manager.executeTrx(this.client, parametersExecute);
		String idRendicion = (String) manager.getDataReturn();
		return idRendicion;
	}

	public String bajaRelacionUsuarioDelegado(String idusr, String delegado) throws TransactionException {
		log.info("Comienza llamado a trx para crear nueva rendicion)");
