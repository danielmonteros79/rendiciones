...
class MotivoObservarActionTest {
    @Mock
    Logger log;
    @Mock
    ...
    @InjectMocks
    MotivoObservarAction motivoObservarAction;
    ...
    void executeAction(HttpServletRequest request, List<ComboMotivo> motivo, ActionMapping mapping, AprobacionForm form) throws Exception {
        try(MockedConstruction<RendicionesService> mock = Mockito.mockConstruction(RendicionesService.class, (mockM, context) -> {
            ...
            assertAll(
                    ...
                    () -> assertEquals("",form.getCmboMotivo()),
                    () -> assertEquals("",form.getMotivoRechazo()),
                    () -> assertEquals("glg",form.getGlg()),
    ...
    private static Stream<Arguments> executeActionSource() {
        ...
        return Stream.of(
                Arguments.of(request,motivo,mapping,form)
        );
    }
